...
- **Real-time alerts.** Uses the SCC v2 notification API with a streaming filter.
- **Readable messages.** Shows category, project, resource, severity, state and event time, with a button that opens the finding in the console.
- **All finding sources.** Security Health Analytics findings show their explanation and recommendation. Threat detection findings show their description and next steps.
- **Project allowlist.** Optionally notify only for selected projects.
...
- **Encrypted secret handling.** The Slack token is committed only as KMS ciphertext and delivered to the function through Secret Manager.
...
| `notification_filter` | see below | SCC findings filter. |
| `allowed_projects` | `[]` | Project IDs or display names to notify for. Empty means all. |
...
To notify only for specific projects, set `allowed_projects`. The function skips findings from other projects.
